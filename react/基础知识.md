...
#### （3）mapStateToProps
是一个函数，将外部的数据（即state对象）转换为UI组件的标签属性
```
...
- 是一个对象，包含了reudx/actions文件里的actions方法名。
- 将分发action的函数转换为UI组件的标签属性

## 六. redux异步编程
redux默认不支持异步代码，如果想在redux中发起请求，需要使用redux的异步插件
### 1. 下载redux插件（异步中间件：中间件即可以扩展默写库功能的插件）
```
...
export const incrementAsync = (number) => {
  return dispatch => {
    // 异步的代码
    setTimeout(() => {
      // increment是一个同步的action，每一个异步的action最好有一个同步的action与之对应
